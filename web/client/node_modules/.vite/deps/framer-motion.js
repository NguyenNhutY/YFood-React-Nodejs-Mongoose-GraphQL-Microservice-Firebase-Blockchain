...
import {
  ...
  x as x2,
  y
} from "./chunk-E5DZTEBX.js";
import {
  Q,
  _,
  k,
  x
} from "./chunk-ZN2J7U5Y.js";
import {
  __export,
  __toCommonJS
...
// node_modules/framer-motion/dist/es/components/AnimatePresence/PopChild.mjs
var PopChildMeasure = class extends x {
...
function PopChild({ children, isPresent: isPresent2 }) {
  ...
  const { nonce } = x2(MotionConfigContext);
  I(() => {
  ...
  return u(PopChildMeasure, { isPresent: isPresent2, childRef: ref, sizeRef: size, children: _n(children, { ref }) });
}
...
var PresenceChild = ({ children, initial, isPresent: isPresent2, onExitComplete, custom, presenceAffectsLayout, mode }) => {
  ...
  const id4 = g();
  const memoizedOnExitComplete = q((childId) => {
  ...
  if (mode === "popLayout") {
    children = u(PopChild, { isPresent: isPresent2, children });
  }
  return u(PresenceContext.Provider, { value: context, children });
};
...
function usePresence(subscribe = true) {
  const context = x2(PresenceContext);
  if (context === null)
    return [true, null];
  ...
  const safeToRemove = q(() => subscribe && onExitComplete && onExitComplete(id4), [id4, onExitComplete, subscribe]);
  return !isPresent2 && onExitComplete ? [false, safeToRemove] : [true];
}
function useIsPresent() {
  return isPresent(x2(PresenceContext));
}
...
// node_modules/framer-motion/dist/es/utils/use-isomorphic-effect.mjs
var useIsomorphicLayoutEffect = isBrowser ? _2 : y;
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, presenceAffectsLayout = true, mode = "sync", propagate = false }) => {
  ...
  const presentChildren = T(() => onlyElements(children), [children]);
  const presentKeys = propagate && !isParentPresent ? [] : presentChildren.map(getChildKey);
  const isInitialRender = A(true);
  const pendingPresentChildren = A(presentChildren);
  ...
  const { forceRender } = x2(LayoutGroupContext);
  return u(k, { children: renderedChildren.map((child) => {
    const key = getChildKey(child);
    ...
    return u(PresenceChild, { isPresent: isPresent2, initial: !isInitialRender.current || initial ? void 0 : false, custom: isPresent2 ? void 0 : custom, presenceAffectsLayout, mode, onExitComplete: isPresent2 ? void 0 : onExit, children: child }, key);
  }) });
};
...
init_compat_module();
var DeprecatedLayoutGroupContext = Q(null);
...
function useForceUpdate() {
  ...
  const deferredForceRender = q(() => frame.postRender(forceRender), [forceRender]);
...
var LayoutGroup = ({ children, id: id4, inherit = true }) => {
  const layoutGroupContext = x2(LayoutGroupContext);
  const deprecatedLayoutGroupContext = x2(DeprecatedLayoutGroupContext);
  ...
  const memoizedContext = T(() => ({ ...context.current, forceRender }), [key]);
  return u(LayoutGroupContext.Provider, { value: memoizedContext, children });
};
...
init_compat_module();
var LazyContext = Q({ strict: false });
...
function LazyMotion({ children, features, strict = false }) {
  const [, setIsLoaded] = d(!isLazyBundle(features));
  const loadedRenderer = A(void 0);
  ...
  return u(LazyContext.Provider, { value: { renderer: loadedRenderer.current, strict }, children });
}
...
function MotionConfig({ children, isValidProp, ...config }) {
  ...
  config = { ...x2(MotionConfigContext), ...config };
  config.isStatic = useConstant(() => config.isStatic);
  const context = T(() => config, [
    JSON.stringify(config.transition),
    config.transformPagePoint,
    config.reducedMotion
  ]);
  return u(MotionConfigContext.Provider, { value: context, children });
}
...
function useCreateMotionContext(props) {
  const { initial, animate: animate2 } = getCurrentTreeVariants(props, x2(MotionContext));
  return T(() => ({ initial, animate: animate2 }), [variantLabelsAsDependency(initial), variantLabelsAsDependency(animate2)]);
}
...
init_compat_module();
var SwitchLayoutGroupContext = Q({});

// node_modules/framer-motion/dist/es/motion/utils/use-visual-element.mjs
function useVisualElement(Component, visualState, props, createVisualElement, ProjectionNodeConstructor) {
  var _a, _b;
  const { visualElement: parent } = x2(MotionContext);
  const lazyContext = x2(LazyContext);
  const presenceContext = x2(PresenceContext);
  const reducedMotionConfig = x2(MotionConfigContext).reducedMotion;
  const visualElementRef = A(null);
  ...
  const initialLayoutGroupConfig = x2(SwitchLayoutGroupContext);
  ...
  const wantsHandoff = A(Boolean(optimisedAppearId) && !((_a = window.MotionHandoffIsComplete) === null || _a === void 0 ? void 0 : _a.call(window, optimisedAppearId)) && ((_b = window.MotionHasOptimisedAnimation) === null || _b === void 0 ? void 0 : _b.call(window, optimisedAppearId)));
  useIsomorphicLayoutEffect(() => {
    if (!visualElement)
      return;
    isMounted.current = true;
...
function createRendererMotionComponent({ preloadedFeatures, createVisualElement, useRender, useVisualState: useVisualState2, Component }) {
  ...
  function MotionComponent(props, externalRef) {
    ...
    return u(MotionContext.Provider, { value: context, children: [MeasureLayout2 && context.visualElement ? u(MeasureLayout2, { visualElement: context.visualElement, ...configAndProps }) : null, useRender(Component, props, useMotionRef(visualState, context.visualElement, externalRef), visualState, isStatic, context.visualElement)] });
  }
  ...
  const ForwardRefMotionComponent = D(MotionComponent);
  ForwardRefMotionComponent[motionComponentSymbol] = Component;
  return ForwardRefMotionComponent;
}
function useLayoutId({ layoutId }) {
  const layoutGroupId = x2(LayoutGroupContext).id;
  return layoutGroupId && layoutId !== void 0 ? layoutGroupId + "-" + layoutId : layoutId;
}
function useStrictMode(configAndProps, preloadedFeatures) {
  const isStrict = x2(LazyContext).strict;
...
var makeUseVisualState = (config) => (props, isStatic) => {
  const context = x2(MotionContext);
  const presenceContext = x2(PresenceContext);
...
function createUseRender(forwardMotionProps = false) {
  const useRender = (Component, props, ref, { latestValues }, isStatic) => {
    ...
    const elementProps = Component !== k ? { ...filteredProps, ...visualProps, ref } : {};
    const { children } = props;
    const renderedChildren = T(() => isMotionValue(children) ? children.get() : children, [children]);
    return _(Component, {
      ...elementProps,
...
// node_modules/framer-motion/dist/es/motion/features/layout/MeasureLayout.mjs
var MeasureLayoutWithContext = class extends x {
  /**
...
function MeasureLayout(props) {
  ...
  const layoutGroup = x2(LayoutGroupContext);
  return u(MeasureLayoutWithContext, { ...props, layoutGroup, switchLayoutGroup: x2(SwitchLayoutGroupContext), isPresent: isPresent2, safeToRemove });
}
...
function useMotionValueEvent(value, event, callback) {
  I(() => value.on(event, callback), [value, event, callback]);
}
...
function calcInset(element, container) {
  ...
  while (current && current !== container) {
    if (current instanceof HTMLElement) {
      ...
      const { x: x3, y: y2 } = current.getBBox();
      inset.x += x3;
      inset.y += y2;
      let svg = null;
...
function useScroll({ container, target, layoutEffect = true, ...options } = {}) {
  ...
  const useLifecycleEffect = layoutEffect ? useIsomorphicLayoutEffect : y;
  useLifecycleEffect(() => {
    refWarning("target", target);
    refWarning("container", container);
    return scroll((_progress, { x: x3, y: y2 }) => {
      values.scrollX.set(x3.current);
      values.scrollXProgress.set(x3.progress);
...
function useMotionValue(initial) {
  ...
  const { isStatic } = x2(MotionConfigContext);
  if (isStatic) {
    const [, setLatest] = d(initial);
    y(() => value.on("change", setLatest), []);
  }
  return value;
}
...
function useSpring(source, config = {}) {
  const { isStatic } = x2(MotionConfigContext);
  const activeSpringAnimation = A(null);
  const value = useMotionValue(isMotionValue(source) ? toNumber(source.get()) : source);
  const latestValue = A(value.get());
  const latestSetter = A(() => {
  });
...
function useReducedMotion() {
  ...
  const [shouldReduceMotion] = d(prefersReducedMotion.current);
  if (true) {
...
function useReducedMotionConfig() {
  ...
  const { reducedMotion } = x2(MotionConfigContext);
...
function useCycle(...items) {
  const index = A(0);
  const [item, setItem] = d(items[index.current]);
  const runCycle = q(
    (next) => {
...
function useInView(ref, { root, margin, amount, once = false } = {}) {
  const [isInView, setInView] = d(false);
  y(() => {
...
function useAnimatedState(initialState) {
  const [animationState, setAnimationState] = d(initialState);
...
var AnimateSharedLayout = ({ children }) => {
  ...
  return u(LayoutGroup, { id: useConstant(() => `asl-${id3++}`), children });
};
...
function useInvertedScale(scale2) {
  ...
  const { visualElement } = x2(MotionContext);
...
function ReorderGroupComponent({ children, as = "ul", axis = "y", onReorder, values, ...props }, externalRef) {
  ...
  return u(Component, { ...props, ref: externalRef, ignoreStrict: true, children: u(ReorderContext.Provider, { value: context, children }) });
}
var ReorderGroup = D(ReorderGroupComponent);
...
function ReorderItemComponent({ children, style = {}, value, as = "li", onDrag, layout: layout2 = true, ...props }, externalRef) {
  const Component = useConstant(() => motion[as]);
  const context = x2(ReorderContext);
  const point2 = {
  ...
  return u(Component, { drag: axis, ...props, dragSnapToOrigin: true, style: { ...style, x: point2.x, y: point2.y, zIndex }, layout: layout2, onDrag: (event, gesturePoint) => {
    const { velocity } = gesturePoint;
    velocity[axis] && updateOrder(value, point2[axis].get(), velocity[axis]);
    onDrag && onDrag(event, gesturePoint);
  }, onLayoutMeasure: (measured) => registerItem(value, measured), ref: externalRef, ignoreStrict: true, children });
}
var ReorderItem = D(ReorderItemComponent);
